...
const custBtn = document.getElementById("custMap");

locateBtn.addEventListener("click", locClick);
checkBtn.addEventListener("click", checkWin);
resetBtn.addEventListener("click", reshuffle);

initDropContainers();
function notify(text, type = "info") { //pod windows na maku nie dziala
...
custBtn.addEventListener("click", customMap);

function inicMap(lat, lon) {
  if (!map) {
    map = L.map("map", {
      zoomControl: true,
...
async function locClick() {
  ...
  navigator.geolocation.getCurrentPosition(
    async (position) => {
      const { latitude, longitude } = position.coords; //dane do init
      inicMap(latitude, longitude);
      try {
    ...
    (error) => {
      ...
      notify(`Error geolokalizacji: ${reason}`, "error");
    },
    {
...
function getLeafletMapImage() {
  return new Promise((resolve, reject) => {
    ...
    //rozmiar mapy na image_size x image_size -> czas render
...
function createTileElement(correctIndex, dataUrl) {
  ...
  tile.title = `Kafelek: ${correctIndex + 1}`;
...
async function customMap() {
  ...
  try {
    const mapImageUrl = await getLeafletMapImage();
    const tiles = await sliceImageToTiles(mapImageUrl, GRID_SIZE);
    currentTiles = tiles;
    renderPuzzle(tiles);
    notify("Układanka wygenerowana z aktualnego widoku mapy.", "success");
  } catch (error) {
    return;
    notify("Nie można wygenerować układanki z mapy. Spróbuj ponownie.", "error");
  }
}
